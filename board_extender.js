(function (window) {
  const CLASS_EXTENDER_ON = "board_extender_on";
  const EXPAND_SVG = `
  ...
  function queryOne(seletor, element = document) {
    return element.querySelector(seletor);
  }

  function queryAll(seletor, element = document) {
    return element.querySelectorAll(seletor);
  }

  function linearBackoff(condition, max = 6, delay = 1000, i = 1) {
